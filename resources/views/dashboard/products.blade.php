@if (count($product))
    @foreach ($product as $pr => $p)
                    ...
                    <div class="row">
                        <div class="col-7">
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="btn-group">
                                    <button data-toggle="modal" data-target="#delete-item{{ $pr }}"
                                        type="button" class="btn btn-sm btn-outline-secondary"><i
                                            class="fa fa-trash-o"></i></button>
                                    <button data-toggle="modal" data-target="#edit-item{{ $pr }}"
                                        type="button" class="btn btn-sm btn-outline-secondary"><i
        ...
        <!-- Modal Delete Item -->
        <div class="modal fade" id="delete-item{{ $pr }}" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <a href="{{ route('delete.product', ['id_produk' => $p->id]) }}"
                            class="btn btn-danger btn-block">Yes</a>
                    </div>
                </div>
            </div>
        ...
        <!-- Modal Edit Item -->
        <div class="modal fade" id="edit-item{{ $pr }}" tabindex="-1" role="dialog"
            aria-labelledby="edit-item" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <form action="{{ route('update.product', ['id' => $p->id]) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="nama_produk" value="{{ $p->nama_produk }}"
                                    class="form-control" id="item" placeholder="Nama item...">
                            </div>

                            <div class="form-group">
                                <label for="harga">Harga</label>
                                <input type="number" name="harga" value="{{ $p->harga }}" class="form-control"
                                    id="harga" placeholder="Harga (Rp)">
                            </div>
...
@else
    ...
    <div class="col-6">
        <div class="blank-page">
            Siap berjualan <br> <i class="fa fa-smile"></i> <br> <a style="width: 50%"
                href="{{ route('add.product') }}" class="btn btn-primary">Ya!</a>
        </div>
    </div>
    <div class="col-3"></div>
@endif
